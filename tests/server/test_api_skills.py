...
async def test_skill_integrity_manifest_rejects_entry_limit_before_read(monkeypatch):
    ...
    root_uri = "viking://user/u1/skills/limited"
    entries = [
        {
            "name": name,
            "uri": f"{root_uri}/{name}",
    ...
    monkeypatch.setattr(skills_router, "_list_skill_files", _list)

    with pytest.raises(ResourceExhaustedError, match="exceeds 1 entries"):
        await skills_router._skill_manifest_with_integrity(
...
async def test_skill_integrity_manifest_stops_after_bounded_batch(monkeypatch):
    from openviking.server.routers import skills as skills_router
    from openviking_cli.exceptions import ResourceExhaustedError
    ...
    class _FS:
        async def read_file_bytes(self, uri, ctx):
            nonlocal active_reads, peak_reads, completed_reads
            del uri, ctx
            active_reads += 1
    ...
    with pytest.raises(ResourceExhaustedError, match="total bytes"):
        await skills_router._skill_manifest_with_integrity(
            types.SimpleNamespace(fs=_FS()),
            types.SimpleNamespace(),
            root_uri,
    ...
    assert peak_reads <= skills_router._SKILL_INTEGRITY_READ_CONCURRENCY
    assert completed_reads == skills_router._SKILL_INTEGRITY_READ_CONCURRENCY


async def test_skill_integrity_manifest_rejects_declared_total_before_read(monkeypatch):
    from openviking.server.routers import skills as skills_router
    from openviking_cli.exceptions import ResourceExhaustedError
...
async def test_skill_snapshot_lock_is_released_on_failure():
    from openviking.server.routers import skills as skills_router

    events = []

    class _AGFS:
        async def pathlock_acquire_tree(self, path, fs_ctx):
            events.append(("acquire", path, fs_ctx))
            return "lease-1"

        async def pathlock_release(self, lease, fs_ctx):
            events.append(("release", lease, fs_ctx))

    class _FS:
        def __init__(self):
            self._async_agfs = _AGFS()

        def _ensure_initialized(self):
            return self

        def _ensure_access(self, uri, ctx):
            events.append(("access", uri, ctx.account_id))

        def _uri_to_path(self, uri, ctx):
            del uri, ctx
            return "/skills/limited"

    ctx = types.SimpleNamespace(account_id="u1")
    with pytest.raises(RuntimeError, match="read failed"):
        async with skills_router._skill_snapshot_lock(
            types.SimpleNamespace(fs=_FS()), ctx, "viking://user/u1/skills/limited"
        ):
            raise RuntimeError("read failed")

    assert events[-1] == ("release", "lease-1", {"account_id": "u1"})
